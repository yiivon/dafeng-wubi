...
bool WriteCustomPhraseFile(const std::vector<DiscoveredWord>& words,
  ...
  std::ofstream f(tmp, std::ios::binary | std::ios::trunc);
  if (!f) {
    DAFENG_LOG_WARN("custom_phrase: open(%s) failed", tmp.c_str());
    return false;
  }

  // Header per librime convention. The columns line is what RIME parses;
  // the comment lines are for human inspection.
  f << "# Rime custom_phrase table — generated by dafeng-daemon\n";
  f << "# Generated at: " << IsoTimestamp() << "\n";
  f << "# Lines: text<TAB>code<TAB>weight\n";
  f << "---\n";
  f << "name: dafeng_learned\n";
  ...
  for (const auto& w : words) {
    if (w.code.empty()) continue;  // RIME needs the key sequence
  ...
  f.close();
  if (!f) {
    DAFENG_LOG_WARN("custom_phrase: write/close failed");
    return false;
  }
  std::error_code ec;
  std::filesystem::rename(tmp, out_path, ec);
  if (ec) {
    DAFENG_LOG_WARN("custom_phrase: rename failed: %s", ec.message().c_str());
    return false;
  }
  DAFENG_LOG_INFO("custom_phrase: wrote %zu entries to %s", emitted,
                   out_path.c_str());
  return true;
}
